...
class PositionEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # vector = torch.zeros(d_model, 1)
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        # apply sin(even) and cos(odd)
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)

        pe = pe.unsqueeze(0) # (1, seq_len, d_model)

        # save file in the buffer
...
class LayerNormalization(nn.Module):
    def __init__(self, eps: float = 10**-6) -> None:
        ...
        self.alpha = nn.Parameter(torch.ones(1)).to('cuda') # multiplied
        self.bias = nn.Parameter(torch.ones(1)).to('cuda') # added

    def forward(self, x):
        mean = x.mean(dim=-1, keepdim=True)
        std = x.std(dim=-1, keepdim=True) # standart deviation

        alpha = self.alpha.to(x.device)
        bias = self.bias.to(x.device)
        # print(alpha.device, bias.device)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, d_model: int, d_ff: int, dropout: float) -> None:
        super().__init__()
        self.dropout = nn.Dropout().to('cuda')
        self.linear1 = nn.Linear(d_model, d_ff).to('cuda')
        self.linear2 = nn.Linear(d_ff, d_model).to('cuda')
...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        self.w_q = nn.Linear(d_model, d_model).to('cuda') # Wq
        self.w_k = nn.Linear(d_model, d_model).to('cuda') # Wk
        self.w_v = nn.Linear(d_model, d_model).to('cuda') # Wv

        self.dropout = nn.Dropout(dropout).to('cuda')

        self.w_o = nn.Linear(d_model, d_model).to('cuda') # Wo
...
class ProjectionLayer(nn.Module):
    def __init__(self, d_model: int, vocab_size: int) -> None:
        super().__init__()
        self.proj = nn.Linear(d_model, vocab_size).to('cuda')
...
class Transformer(nn.Module):
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: InputEmbeddings, tgt_embed: InputEmbeddings,
                 src_pos: PositionEncoding, tgt_pos: PositionEncoding, projection_layer: ProjectionLayer) -> None:
        super().__init__()
